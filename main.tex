  	...
  	\subsection{Air Resistance}
        ...
        \]
  		...
  		\begin{multicols}{2}
  			...
  			\vfill
  		\columnbreak
  			\noindent{Net force for $m_1$ (smaller block):}
  			\[
            	\Sigma\vec{F}=m_1\vec{a}=T-m_1 gsin(\theta)-\mu_km_1gcos(\theta)
            \]
  			Net force for $m_2$ (larger block):
  			\[
            	\Sigma\vec{F}=m_2\vec{a}=m_2gsin(\theta)-T-\mu_km_1gcos(\theta)-\mu_k(m_1+m_2)gcos(\theta)
            \]
       	\end{multicols}
